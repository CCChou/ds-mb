...
public class ArticleDao {
	...
	@Transactional
	public void create(UserBean user, String title, String content) {
		sessionFactory.getCurrentSession().save(new ArticleBean(user, title, content));
	}
	...
	public long findCountByTitleCondition(String condition) {
		Query query = sessionFactory.getCurrentSession()
				.createQuery("select count(*) from ArticleBean where title like ?")
	...
	public List<ArticleBean> findTitleRangeByCondition(String condition, int start, int limit) {
		Query query = sessionFactory.getCurrentSession()
				.createQuery("from ArticleBean where title like ? order by date desc")
				.setParameter(0, "%" + condition + "%").setFirstResult(start).setMaxResults(limit);
		return query.list();
	}
